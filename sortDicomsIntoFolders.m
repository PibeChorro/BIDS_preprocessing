...
%   "log" containing the fields: sequenceDescriptions, sequenceNames, and
%   sequenceScanNrs.
% 
...
%           formatSpec: format specification for RUNS (default: %0.2i).
%                       E.g. run-01, -02, etc.
...
if isfield(params, 'sourceDir'); sourceDir = params.sourceDir; else
    fprintf('====SOURCEDIR: No default. Please specify the directory that contains your subjectfolders (ideally it should be named "sourcedata")\n\n')
...
if ~isfield(log, 'sequenceDescriptions') || ~isfield(log, 'sequenceNames') || ~isfield(log, 'sequenceScanNrs')
...
for f = 1:length(sub) % Outer loop for the subjects
    ...
    if isempty(dicoms)
        ...
        elseif sum(isfolder(arrayfun(@(x) fullfile(sourceDir, sub(1).name, x.name), folderContent, 'UniformOutput', false))) > 3 % Check if you have any folders (apart from '.' and '..')
    ...
    for sd = 1:length(subDirs)
        ...
        if status
            ...
            if (contains(log.sequenceNames{sequenceIndex},'anat')||contains(log.sequenceNames{sequenceIndex},'struct'))
                ...
                if ismember(nrOfScans, log.sequenceScanNrs{sequenceIndex}) % check for the correct number of scans
        ...
        for sd = 1:length(subDirs)
            ...
            if status
                ...
                if together % put files into the sequenceNames folder; ignore the wanted number of scans
                    movefile(string(currentDir), fullfile(subjectDir,log.sequenceNames{i}));
                else
